...
class NaturalToILRParser(ABC):
    ...
    @abstractmethod
    def parse_async(self, input: NaturalLanguageInput) -> Result[ILRNode, str]:
...
class ILRToTCETransformer(ABC):
    ...
    @abstractmethod
    def transform_async(self, ilr: ILRNode) -> Result[List[TCEStatement], str]:
...
class TCEToTauGenerator(ABC):
    ...
    @abstractmethod
    def generate_async(self, tce: List[TCEStatement]) -> Result[TauProgram, str]:
...
class ParserPipelineOrchestrator:
    ...
    async def parse_natural_to_tau_async(self, text: str) -> Result[TauProgram, str]:
    ...
    async def parse_natural_to_tce_async(self, text: str) -> Result[List[TCEStatement], str]:
        """Partial pipeline: Natural Language → TCE."""
        input_data = self._prepare_input(text)
        ilr_result = await self._parse_to_ilr_async(input_data)
        return await self._transform_ilr_to_tce_async(ilr_result)
    
    async def parse_tce_to_tau_async(self, tce: List[TCEStatement]) -> Result[TauProgram, str]:
    ...
    async def _parse_to_ilr_async(self, input: NaturalLanguageInput) -> Result[ILRNode, str]:
        """Parse to ILR with error handling."""
        return await self._nl_parser.parse_async(input)
    
    async def _process_ilr_to_tau_async(self, ilr_result: Result[ILRNode, str]) -> Result[TauProgram, str]:
    ...
    async def _transform_ilr_to_tce_async(self, ilr_result: Result[ILRNode, str]) -> Result[List[TCEStatement], str]:
    ...
    async def _generate_tau_from_tce_async(self, tce_result: Result[List[TCEStatement], str]) -> Result[TauProgram, str]:
