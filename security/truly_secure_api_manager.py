...
def test_truly_secure_manager():
    ...
    try:
        ...
        # Test password setup - use environment variable or generate
        import os
        import secrets
        import string
        
        test_password = os.getenv('TEST_PASSWORD', ''.join(secrets.choice(string.ascii_letters + string.digits + "!@#$%") for _ in range(16)))
        print(f"Setting up with secure test password")
        
        if manager.initialize_encryption(test_password):
            print("✅ Encryption initialized successfully")
            
            # Test key storage - use mock key for testing
            test_key = "sk-test-" + ''.join(secrets.choice(string.ascii_letters + string.digits) for _ in range(20))
