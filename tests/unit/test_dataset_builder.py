...
import json
from backend.unified.dataset.builder import load_seed_yaml, paraphrase_offline, validate_tce_sym_deterministic, DataItem


def test_load_seed_yaml_and_paraphrase(tmp_path):
    seeds = tmp_path / "seeds.yaml"
    seeds.write_text("- nl: If A then B\n  tce_en: At all times, if A then B.\n  tce_sym: always (A -> B)\n", encoding="utf-8")
...
def test_validate_tce_deterministic_accepts_simple_implication():
    ok, reasons = validate_tce_sym_deterministic("always (A -> B)")
    assert ok is True
...
def test_save_pairs_jsonl(tmp_path):
    ...
    data = [DataItem(nl="A.", tce_en="At all times, A.", tce_sym="always (A)")]
    from backend.unified.dataset.builder import save_pairs_jsonl
    save_pairs_jsonl(data, str(out))
    lines = out.read_text(encoding="utf-8").strip().splitlines()
    obj = json.loads(lines[0])
    assert obj["tce_sym"].startswith("always (")
    assert obj["tce_en"].lower().startswith("at all times")
